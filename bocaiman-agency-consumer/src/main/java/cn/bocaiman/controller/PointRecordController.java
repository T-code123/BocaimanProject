...
/**
 * <b>菠菜侠旅游租赁平台-旅行社管理-积分记录控制器</b>
 * @author Arthur
 * @version 1.0.0
 * @since 1.0.0
 */
@Api(value = "菠菜侠旅游租赁平台-旅行社管理-积分记录控制器", tags = {"菠菜侠旅游租赁平台-旅行社管理-积分记录交互接口"})
@RestController("pointRecordController")
@RequestMapping("/travelagency/pointrecord")
public class PointRecordController extends BaseController {
	@Autowired
	private PointRecordTransport transport;
	...
	@ApiImplicitParam(value = "查询视图", dataType = "PointRecordVO", required = true)
	@ApiResponse(code = 200, message = "系统响应视图")
	@PostMapping("/list")
	public ResponseVO queryList(@RequestBody PointRecordVO queryVO) throws Exception {
	...
	public ResponseVO queryByOrderNo(@PathVariable("orderNo") String orderNo) throws Exception {
		// 封装查询对象
		PointRecordVO queryVO = new PointRecordVO();
		queryVO.setOrderNo(orderNo);
		// 查询列表
		List<PointRecordVO> voList = transport.getByList(queryVO);
	...
	@ApiParam(value = "保存视图", type = "PointRecordVO", required = true)
	@ApiResponse(code = 200, message = "系统响应视图")
	@PostMapping("/save")
	public ResponseVO save(@RequestBody PointRecordVO vo) throws Exception {
		// 信息是否有效
		if (vo.getOrderNo() != null && !"".equals(vo.getOrderNo().trim()) && vo.getCurrentTotalPoint() != null
				&& !"".equals(vo.getCurrentTotalPoint())) {
			// 查询订单编号信息
			PointRecordVO queryVO = new PointRecordVO();
			queryVO.setOrderNo(vo.getOrderNo());
			List<PointRecordVO> list = transport.getByList(queryVO);
			// 查询剩余积分信息
			queryVO.setOrderNo(null);
			queryVO.setCurrentTotalPoint(vo.getCurrentTotalPoint());
			List<PointRecordVO> list1 = transport.getByList(queryVO);
