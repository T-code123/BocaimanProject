...
/**
 * <b>菠菜侠旅游租赁平台-订单管理-订单退款控制器</b>
 * @author Arthur
 * @version 1.0.0
 * @since 1.0.0
 */
@Api(value = "菠菜侠旅游租赁平台-订单管理-订单退款控制器", tags = "菠菜侠旅游租赁平台-订单管理-订单退款交互接口")
@RestController("OrderRefundRecordController")
@RequestMapping("/order/refund")
public class OrderRefundRecordController extends BaseController {
	@Autowired
	private OrderRefundRecordTransport transport;
	...
	@ApiImplicitParam(value = "查询视图", dataType = "OrderRefundRecordVO", required = true)
	@ApiResponse(code = 200, message = "系统响应视图")
	@PostMapping("/list")
	public ResponseVO queryList(@RequestBody OrderRefundRecordVO queryVO) throws Exception {
	...
	public ResponseVO queryByOrderNo(@PathVariable("orderNo") String orderNo) throws Exception {
		// 封装查询对象
		OrderRefundRecordVO queryVO = new OrderRefundRecordVO();
		queryVO.setOrderNo(orderNo);
		// 查询列表
		List<OrderRefundRecordVO> voList = transport.getByList(queryVO);
	...
	@ApiParam(value = "保存视图", type = "OrderRefundRecordVO", required = true)
	@ApiResponse(code = 200, message = "系统响应视图")
	@PostMapping("/save")
	public ResponseVO save(@RequestBody OrderRefundRecordVO vo) throws Exception {
		// 信息是否有效
		if (vo.getOrderNo() != null && !"".equals(vo.getOrderNo().trim()) && vo.getOrderNo() != null
			...
			List<OrderRefundRecordVO> list = transport.getByList(queryVO);
			// 查询审核状态信息
			queryVO.setAuditStatus(null);
			queryVO.setAuditStatus(vo.getAuditStatusId());
			List<OrderRefundRecordVO> list1 = transport.getByList(queryVO);
